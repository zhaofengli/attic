...
use chrono::Utc;
use sea_orm::{
    ConnectionTrait, Database, DatabaseConnection, EntityTrait, IntoActiveModel, Set,
    query::Statement,
};
...
pub async fn run_migrations(config: Config) -> Result<()> {
    ...
    for cache in db_caches {
        if let Some(configured) = state.config.caches.get(&cache.name) {
            ...
            if let Some(keypair) = &configured.keypair
                && *keypair != cache.keypair
            {
                let new = NixKeypair::from_str(keypair)?;
                update.keypair = Set(new.export_keypair());
                modified = true;
            }
            if modified {
        ...
        .filter(|(name, _)| !exists.contains(name.as_str()))
    {
        ...
        let keypair = if let Some(keypair) = &config.keypair {
            NixKeypair::from_str(keypair)?
        } else {
